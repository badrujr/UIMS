...
if (isset($_POST['faculty'])) {
  ...
  $check_uni = "SELECT * FROM faculty WHERE f_name = '$fname'";
  ...
  if ($count == 1) {
    # code...
     $error_check = "<div class='alert alert-danger alert-has-icon'>
                         We Already have that $fname information/profile, please go to manage faculty and proceed with other details ...
                         </div>";
  }
   ...
   $cam = "SELECT campus.campus_id,campus.name,university.university_name FROM campus INNER JOIN university ON campus.university_id = university.id WHERE campus.name = '$cname'";
    $run_cam = mysql_query($cam);
    $info = mysql_fetch_array($run_cam);
    $uid = $info['campus_id'];
  
  $sql = "INSERT INTO faculty (fname,created_at,c_id) VALUES ('$fname','$created_at','$uid')";
  $run = mysql_query($sql);

  if ($run) {
    $msg = "<div class='alert alert-success alert-has-icon'>
                         Faculty details have been added successfully...
                         </div>";
                         header('Refresh:3; URL=manage-faculty.php');
  }

  else{
    $error = "<div class='alert alert-danger alert-has-icon'>
                         Unable to add Faculty details...
                         </div>";
  }
  ...
  <div id="app">
    <div class="main-wrapper main-wrapper-1">
     ...
     require_once('dbconnection.php');
      ...
      $sqll = "SELECT campus.campus_id,campus.name,university.university_name FROM campus INNER JOIN university ON campus.university_id = university.id WHERE university.university_name = '$chuo'";
      ...
      while ($row_campus = mysql_fetch_array($run_sqll)) {
          $c_id = $row_campus['c_id'];
          $c_name = $row_campus['name'];

          echo "<option>$c_name</option>";
        }
        echo "</select>
          </div>
        </div>";
       ...
       <div class='form-group row mb-4'>
        ...
        <div class='form-group row mb-4'>
          <label class='col-form-label text-md-right col-12 col-md-3 col-lg-3'></label>
          <div class='col-sm-12 col-md-7'>
            <input type='submit' name='faculty' class='btn btn-primary' value='create-faculty'>
          </div>
        </div>";
  }              
