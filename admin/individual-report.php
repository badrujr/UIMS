              ...
              <div class="col-12">
                <div class="card">
                  <div class="card-header">
                    <h4>Individual Report For Particular University/Institute/College:</h4>
                  </div>
                  <div class="card-body">
                    <?php echo $msg; echo $error2;?>
                    <form method="POST" action="">
                    <div class="form-group row mb-4">
                      <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Name:</label>
                      <div class="col-sm-12 col-md-7">
                        <select class="form-control selectric" name="uni">
                          <option>--select--</option>
                          <?php
                        require_once('dbconnection.php');
                        $count = "SELECT DISTINCT university_name FROM university ORDER BY university_name";
                        $query = mysql_query($count);
                      while ($rows = mysql_fetch_array($query)) {
                        $majina = $rows['university_name'];
                         echo "<option>$majina</option>";
                      }
                    ...
                    <div class="form-group row mb-4">
                      ...
                      <div class="col-sm-12 col-md-7">
                        <input type="submit" name="save" class="btn btn-primary" value="Create-Report">
                      </div>
                    </div>
          <hr style="border-color: red;">
            <?php
             require_once('dbconnection.php');
                        ...
                        $x = 0;
                        if ($uni =='--select--') {
                            $act = "Trying to print reports for $uni";
                            $dt=date("y-m-d h:i:s");
                            $invx="  INSERT INTO `visited` (`id`, `fname`, `title`, `action`,`date_time`) VALUES (NULL, '$fname', '$tit','$act','$dt')";
                            $iser=mysql_query($invx);
                          echo"
                          <div class='alert alert-danger alert-dismissible'>
                          <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button> Please select any University/Institute/College because <b>$uni</b> is not allowed...
                          </div>";     
                        }

                        else{
                          $hide = base64_encode($uni);
                                 echo"
                         <div class='row'>
                         <div class='col-12'>
                        <div class='card'>
                        <div class='card-header'>
                         <h4>More Information About: <b style = 'color: green; font-size: 20px;'>$uni</b></h4>
                       </div>
                      <div class='card-body'>
                      <a href='collegeIndPreview.php?xxx=$hide' style = 'float:right;' class = 'btn btn-primary'><i class = 'fas fa-eye'>&nbsp;</i>Preview</a>
                      <br>
                      <br>
                      <br>
                    <div class='table-responsive'>
                      <table class='table table-striped table-hover' id='save-stage' style='width:100%;'>
                        ...
                        <tbody>";    
                        while ($rows = mysql_fetch_array($run_sqll)) {
                                  $id = $rows['id'];
                                  $fname = $rows['fname'];
                                  $lo = $rows['location'];
