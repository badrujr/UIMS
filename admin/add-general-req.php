...
if (isset($_POST['requi'])) {
  $un_id = mysql_escape_string(trim(strip_tags($_POST['uname'])));
  ...
  $created_at = date('Y/m/d H:i:s');
   ...
   $check = "SELECT * FROM university WHERE university_name = '$un_id'";
   $run_check = mysql_query($check);
   $rows_data = mysql_fetch_array($run_check); 
   $uid = $rows_data['id'];


    $sql = "INSERT INTO gen_requirement (university_id,content,created_at) VALUES ('$uid','$req','$created_at')";
    $run = mysql_query($sql);

  if ($run) {
    $msg = "<div class='alert alert-success alert-has-icon'>
                          Requirement have been added successfully...
                         </div>";
                         header('Refresh:3; URL=manage-all-req.php');
  }
                  ...
                  <div class="card-body">
                    ...
                    <form method="POST" action="">
                     ...
                     <div class='form-group row mb-4'>
                      ...
                      <div class='col-sm-12 col-md-7'>
                        <textarea class='form-control' name='all-req' id = 'editor'></textarea>
                      </div>
                    </div>
        <div class='form-group row mb-4'>
          <label class='col-form-label text-md-right col-12 col-md-3 col-lg-3'></label>
          <div class='col-sm-12 col-md-7'>
            <input type='submit' name='requi' class='btn btn-primary' value='all-requirement'>
          </div>
        </div>
