...
if (isset($_POST['save'])) {
  ...
  if ($uname == "--select--") {
    ...
    $uni = "SELECT * FROM university WHERE university_name = '$uname'";
    $run_uni = mysql_query($uni);
    $info = mysql_fetch_array($run_uni);
    $uid = $info['id'];
  
    $sql = "INSERT INTO campus (name,created_at,university_id) VALUES ('$cname','$created_at','$uid')";
  $run = mysql_query($sql);

  if ($run) {
    $msg = "<div class='alert alert-success alert-has-icon'>
                         Campus details have been added successfully...
                         </div>";
                         header('Refresh:3; URL=manage-cam.php');
  }

  else{
    $error = "<div class='alert alert-danger alert-has-icon'>
                         Unable to add campus details...
                         </div>";
  }
                ...
                <div class="card">
                  ...
                  <div class="card-body">
                    ...
                    <div class="form-group row mb-4">
                      ...
                      <div class="col-sm-12 col-md-7">
                        <input type="submit" name="save" class="btn btn-primary" value="Create-campus">
                      </div>
                    </div>
                    </form>
