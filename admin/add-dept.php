...
if (isset($_POST['dept'])) {
  ...
  $created_at = date('Y/m/d H:i:s');
   ...
   $check_uni = "SELECT * FROM department WHERE d_name = '$dname'";
  ...
  if ($count == 1) {
    # code...
     $error_check = "<div class='alert alert-danger alert-has-icon'>
                         We Already have that $dname information/profile, please go to manage department and proceed with other details ...
                         </div>";
  }
    ...
    $fac = "SELECT campus.campus_id,campus.name,faculty.fname,faculty.id FROM campus INNER JOIN faculty ON campus.campus_id = faculty.c_id WHERE faculty.fname = '$fname'"; 
    $run_fac = mysql_query($fac);
  
    $info = mysql_fetch_array($run_fac);
    $uid = $info['campus_id'];
    $fid = $info['id'];
  
    $sql = "INSERT INTO department (d_name,created_at,faculty_id) VALUES ('$dname','$created_at','$fid')";
    $run = mysql_query($sql);

  if ($run) {
    $msg = "<div class='alert alert-success alert-has-icon'>
                         Department details have been added successfully...
                         </div>";
                         header('Refresh:3; URL=manage-dept.php');
  }

  else{
    $error = "<div class='alert alert-danger alert-has-icon'>
                         Unable to add department details...
                         </div>";
  }
...
<head>
  ...
  <title>uims - Add University</title>
  <!-- General CSS Files -->
  <!-- General CSS Files -->
  ...
  <div id="app">
    <div class="main-wrapper main-wrapper-1">
     ...
     require_once('dbconnection.php');
      ...
      $sqll = "SELECT DISTINCT name FROM university,campus,faculty WHERE university.id = campus.university_id AND campus.campus_id = faculty.c_id AND university.university_name = '$chuo'";
      $run_sqll = mysql_query($sqll);

      $sqlx = "SELECT DISTINCT fname FROM university,campus,faculty WHERE university.id = campus.university_id AND campus.campus_id = faculty.c_id AND university.university_name = '$chuo'";
      ...
      echo "<div class='form-group row mb-4'>
        ...
        <div class='form-group row mb-4'>
          <label class='col-form-label text-md-right col-12 col-md-3 col-lg-3'></label>
          <div class='col-sm-12 col-md-7'>
            <input type='submit' name='dept' class='btn btn-primary' value='create-department'>
          </div>
        </div>";
  }              
