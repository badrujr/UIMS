  ...
  if ($count == 1) {
    # code...
     $error_check = "<div class='alert alert-danger alert-has-icon'>
                         We Already have that $uname information/profile, please go to manage university and proceed with other details ...
                         </div>";
  }

  else if ($org == "--select--") {
      $error2 = "<div class='alert alert-danger alert-has-icon'>
                         Please select any Organization type, because $org is not allowed...
                         </div>";
  }

  else{

    $org = "SELECT * FROM organization_type WHERE org_name = '$org'";
    $run_org = mysql_query($org);
    $info = mysql_fetch_array($run_org);
    $oids = $info['org_type_id'];
  
    $sql = "INSERT INTO university (university_name,location,org_type_id,website,contact,phy_address) VALUES ('$uname','$lo','$oids','$web','$cont','$phy')";
  $run = mysql_query($sql);

  if ($run) {
    $msg = "<div class='alert alert-success alert-has-icon'>
                         University details have been added successfully...
                         </div>";
    ...
    $error = "<div class='alert alert-danger alert-has-icon'>
                         Unable to add university details...
                         </div>";
  }
                ...
                <div class="card">
                  ...
                  <div class="card-body">
                    ...
                    <div class="form-group row mb-4">
                      ...
                      <div class="col-sm-12 col-md-7">
                        <input type="submit" name="save" class="btn btn-primary" value="Create-university">
                      </div>
                    </div>
                    </form>
